...
function CompetitionsList() {
  ...
  const handleScreensDialogItemClick = () => {
    closeScreensDialog();
  };
  ...
  const columns = useMemo(
    () => [
      ...
      { field: 'countryName', header: 'Country', width: 100, render: (value, row) => row.countryName || row.country || '-' },
      { field: 'sport', header: 'Sport', width: 90 },
      {
        field: 'GENDER',
        header: 'Gender',
        width: 90,
      ...
      },
      {
        ...
        header: 'Type',
        width: 100,
        render: (value, row) => competitionTypes.find((ct) => ct.COMPETITION_TYPE_ID === row.COMPETITION_TYPE)?.COMPETITION_TYPE || '—',
      },
      {
        field: 'MAIN_COLOR',
        header: 'Main Color',
        ...
        render: (value, row) =>
          isEditMode ? (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <input
                type="color"
                value={numberToHex(pendingChanges[row.COMPETITION_ID]?.MAIN_COLOR ?? row.MAIN_COLOR) || '#000000'}
                onChange={(e) => handleFieldChange(row.COMPETITION_ID, 'MAIN_COLOR', hexToNumber(e.target.value))}
                style={{ width: 28, height: 28, padding: 0, border: '1px solid #ddd', borderRadius: 4, cursor: 'pointer' }}
              />
              <Typography variant="body2" sx={{ minWidth: 68 }}>{numberToHex(pendingChanges[row.COMPETITION_ID]?.MAIN_COLOR ?? row.MAIN_COLOR) || '-'}</Typography>
            </Box>
          ) : (
            row.MAIN_COLOR != null ? (
              <Box
                sx={{
                  ...
                  bgcolor: numberToHex(row.MAIN_COLOR) || 'transparent',
                  flexShrink: 0,
                }}
                title={numberToHex(row.MAIN_COLOR)}
              />
            ) : (
              '—'
            )
      ...
      { field: 'pid', header: 'PID', width: 50 },
      {
        field: 'HIDE_ON_CATALOG',
        header: 'Hide Catalog',
        width: 90,
        render: (value, row) => (row.HIDE_ON_CATALOG === true ? 'Yes' : row.HIDE_ON_CATALOG === false ? 'No' : '—'),
      },
      {
        field: 'HIDE_ON_SEARCH',
        header: 'Hide Search',
        width: 90,
        render: (value, row) => (row.HIDE_ON_SEARCH === true ? 'Yes' : row.HIDE_ON_SEARCH === false ? 'No' : '—'),
      },
      {
        field: 'ENABLE_DASHBOARD_BUZZ',
        header: 'Dashboard Buzz',
        width: 100,
        render: (value, row) => (row.ENABLE_DASHBOARD_BUZZ === true ? 'Yes' : row.ENABLE_DASHBOARD_BUZZ === false ? 'No' : '—'),
      },
      { field: '_', header: 'Bet Lines', width: 80, noFilter: true, render: () => '—' },
      {
        field: '_',
        header: 'Screens',
        width: 70,
    ...
    [isEditMode, pendingChanges, handleNameClick, handleFieldChange, openScreensDialog, numberToHex, hexToNumber, competitionTypes]
  );
                ...
                groupedAndPaginated.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {columns.map((col) => (
                        <TableCell key={col.field + (col.header || '')} sx={{ borderColor: '#EAECF0', width: col.width }} onClick={col.field === 'GENDER' || col.field === 'MAIN_COLOR' || col.field === '_' ? (e) => e.stopPropagation() : undefined}>
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
            {[
              ...
              { label: 'Priorities', path: screensDialogCompetition ? `/competitions/${screensDialogCompetition.COMPETITION_ID}/priorities` : '' },
              { label: 'Bet Lines', path: screensDialogCompetition ? `/competitions/${screensDialogCompetition.COMPETITION_ID}/bet-lines` : '' },
              ...
              <Button
                ...
                fullWidth
                variant="text"
                onClick={handleScreensDialogItemClick}
