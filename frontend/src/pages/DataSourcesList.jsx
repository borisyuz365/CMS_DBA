...
function DataSourcesList() {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    Object.keys(columnFilters).forEach((columnField) => {
      ...
      if (filterValue && filterValue.trim() !== '') {
        ...
        filtered = filtered.filter((row) => {
          ...
          if (columnField === 'DATA_SOURCE_ID' || columnField === 'LANG_ID' || columnField === 'UI_LANG_ID' || columnField === 'LOG_OPTION_ID') {
    ...
    if (filters.dataSourceId) {
      filtered = filtered.filter((s) => String(s.DATA_SOURCE_ID) === String(filters.dataSourceId));
    }
    ...
    return filtered;
  }, [sources, columnFilters, sortConfig, filters.dataSourceId, filters.aliasName, showDeleted]);

  const dataForTable = hasSearched ? filteredAndSortedSources : [];
  ...
  const handleFieldChange = (dataSourceId, field, value) => {
    setPendingChanges((prev) => {
      const next = { ...prev };
      if (!next[dataSourceId]) next[dataSourceId] = {};
      next[dataSourceId][field] = value;
      return next;
    });
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const updates = Object.entries(pendingChanges).map(([dataSourceId, changes]) => ({
        dataSourceId: parseInt(dataSourceId, 10),
        changes,
      }));
      const result = await api.updateDataSourcesBulk(updates);
      const msg = result.errors && result.errors.length > 0
        ? `Updated ${result.updated} data source(s). ${result.errors.length} error(s).`
        : `Updated ${result.updated} data source(s).`;
  ...
  const handleOpenCreateDialog = () => {
    setCreateFormData({
      ...
      LANG_ID: null,
      UI_LANG_ID: null,
      TIME_ZONE_ID: timeZones.length ? timeZones[0].TIME_ZONE_ID : null,
      ALWAYS_ADD_VALUE_IN_LANG: null,
      ADD_TO_LOG: null,
      WIDGET_ACTIVE: null,
      LOG_OPTION_ID: null,
  ...
  const validateCreateForm = () => {
    ...
    if (!createFormData.ALIAS_NAME || !createFormData.ALIAS_NAME.trim()) {
      errors.ALIAS_NAME = 'Data source name (ALIAS_NAME) is required';
    }
  ...
  const handleCreateDataSource = async () => {
    ...
    try {
      ...
      const newTerm = await api.createTerm({
        category: 'Data Source names',
        values: [
          { languageId: 1, value: aliasTrimmed, isDefault: true, status: 'Approved' },
        ],
      });
      const newItem = await api.createDataSource({
        NAME_ID: newTerm.id,
        ALIAS_NAME: aliasTrimmed,
        SITE_ADDRESS: createFormData.SITE_ADDRESS?.trim() || null,
        LANG_ID: createFormData.LANG_ID != null && createFormData.LANG_ID !== '' ? Number(createFormData.LANG_ID) : null,
        UI_LANG_ID: createFormData.UI_LANG_ID != null && createFormData.UI_LANG_ID !== '' ? Number(createFormData.UI_LANG_ID) : null,
        TIME_ZONE_ID: createFormData.TIME_ZONE_ID != null && createFormData.TIME_ZONE_ID !== '' ? Number(createFormData.TIME_ZONE_ID) : (timeZones[0]?.TIME_ZONE_ID ?? null),
        ALWAYS_ADD_VALUE_IN_LANG: createFormData.ALWAYS_ADD_VALUE_IN_LANG === true || createFormData.ALWAYS_ADD_VALUE_IN_LANG === false ? createFormData.ALWAYS_ADD_VALUE_IN_LANG : null,
        ADD_TO_LOG: createFormData.ADD_TO_LOG === true || createFormData.ADD_TO_LOG === false ? createFormData.ADD_TO_LOG : null,
        WIDGET_ACTIVE: createFormData.WIDGET_ACTIVE === true || createFormData.WIDGET_ACTIVE === false ? createFormData.WIDGET_ACTIVE : null,
        LOG_OPTION_ID: createFormData.LOG_OPTION_ID != null && createFormData.LOG_OPTION_ID !== '' ? Number(createFormData.LOG_OPTION_ID) : null,
      });
      handleCloseCreateDialog();
      await loadDataSources();
      setSnackbar({ open: true, message: 'Data source created successfully', severity: 'success' });
      navigate(`/data-sources/${newItem.DATA_SOURCE_ID}`);
    } catch (err) {
      console.error('Failed to create data source:', err);
      setSnackbar({ open: true, message: err.message || 'Failed to create data source', severity: 'error' });
      setError(err.message);
    } finally {
      setLoading(false);
    }
  ...
  const hasDeletedSelected = useMemo(() => {
    return selectedRows.some((id) => {
      const s = sources.find((x) => x.DATA_SOURCE_ID === id);
      return s?.IS_DELETED === true;
    });
  }, [selectedRows, sources]);

  const handleDeleteOrRestore = () => {
    if (selectedRows.length === 0) {
      setSnackbar({ open: true, message: 'Please select data sources', severity: 'warning' });
      return;
    }
    ...
    setConfirmDialog({
      open: true,
      title: `${actionText.charAt(0).toUpperCase() + actionText.slice(1)} Data Sources`,
      message: `Are you sure you want to ${actionText} ${selectedRows.length} data source(s)?`,
      onConfirm: async () => {
        try {
          setLoading(true);
          setError(null);
          if (isRestore) {
            await api.restoreDataSources(selectedRows);
            setSnackbar({
              open: true,
              message: `Successfully restored ${selectedRows.length} data source(s)`,
              severity: 'success',
            });
          } else {
            await api.deleteDataSources(selectedRows);
            setSnackbar({
              open: true,
              message: `Successfully deleted ${selectedRows.length} data source(s)`,
              severity: 'success',
            });
          }
          setSelectedRows([]);
          ...
          console.error(`Failed to ${actionText} data sources:`, err);
          setSnackbar({
            open: true,
            message: `Failed to ${actionText} data sources. Please try again.`,
            severity: 'error',
          });
          setError(err.message || `Failed to ${actionText} data sources`);
        } finally {
          setLoading(false);
  ...
  const handleExportCsv = () => {
    const rows = selectedRows.length > 0
      ? dataForTable.filter((r) => selectedRows.includes(r.DATA_SOURCE_ID))
      : dataForTable;
    if (rows.length === 0) {
    ...
    a.href = url;
    a.download = `data_sources_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    URL.revokeObjectURL(url);
    setSnackbar({ open: true, message: `Exported ${rows.length} data source(s)`, severity: 'success' });
  };
  ...
  const columns = useMemo(
    () => [
      {
        ...
        render: (value) => (
          <Typography
            ...
            onClick={(e) => { e.stopPropagation(); window.open(`/data-sources/${value}`, '_blank'); }}
          >
            {value}
          </Typography>
        ),
            ...
            <Typography
              ...
              onClick={(e) => {
                ...
                if (row.NAME_ID) {
                  ...
                  window.open(`/data-sources/${row.DATA_SOURCE_ID}`, '_blank');
                }
              }}
            >
              {value || '-'}
      ...
      },
      {
        ...
        width: '22ch',
        render: (value) => (
          <Typography sx={{ fontSize: '0.875rem', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '22ch' }} title={value || ''}>
            {value || '-'}
          </Typography>
        ),
      },
      ...
      },
      {
        ...
        render: (value) => {
          ...
          const tz = timeZones.find((t) => t.TIME_ZONE_ID === value || t.TIME_ZONE_ID === Number(value));
          return tz ? formatTimeZoneDisplay(tz) : String(value);
        },
      },
      {
        field: 'ALWAYS_ADD_VALUE_IN_LANG',
        header: 'Add Value in Lang',
        width: 110,
        render: (value, row) => {
          if (isEditMode) {
            const v = pendingChanges[row.DATA_SOURCE_ID]?.ALWAYS_ADD_VALUE_IN_LANG ?? value;
            return (
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={!!v}
                    onChange={(e) => handleFieldChange(row.DATA_SOURCE_ID, 'ALWAYS_ADD_VALUE_IN_LANG', e.target.checked)}
                  />
                }
                label=""
              />
      ...
      },
      {
        ...
        render: (value, row) => {
          if (isEditMode) {
            const v = pendingChanges[row.DATA_SOURCE_ID]?.ADD_TO_LOG ?? value;
            return (
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={!!v}
                    onChange={(e) => handleFieldChange(row.DATA_SOURCE_ID, 'ADD_TO_LOG', e.target.checked)}
                  />
                }
                label=""
              />
      ...
      },
      {
        ...
        render: (value, row) => {
          if (isEditMode) {
            const v = pendingChanges[row.DATA_SOURCE_ID]?.WIDGET_ACTIVE ?? value;
            return (
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={!!v}
                    onChange={(e) => handleFieldChange(row.DATA_SOURCE_ID, 'WIDGET_ACTIVE', e.target.checked)}
                  />
                }
                label=""
              />
    ...
    [isEditMode, pendingChanges, timeZones]
  );
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', p: 3, backgroundColor: '#f5f5f5', overflow: 'auto' }}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6} md={2.4}>
            <TextField
              ...
              onChange={(e) => handleFilterChange('dataSourceId', e.target.value)}
              sx={{
      ...
      <Paper sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: 2, backgroundColor: '#ffffff' }}>
                ...
                <TableRow sx={{ position: 'sticky', top: 0, zIndex: 11 }}>
                  <TableCell padding="checkbox" sx={{ backgroundColor: '#ffffff', borderColor: '#EAECF0', position: 'sticky', top: 0, zIndex: 12 }}>
                    <Checkbox
                      ...
                      onChange={(e) => (e.target.checked ? setSelectedRows(paginatedData.map((r) => r.DATA_SOURCE_ID)) : setSelectedRows([]))}
                      sx={{ padding: '4px' }}
                    />
                  </TableCell>
                  {columns.map((col) => (
                  ...
                  <TableRow>
                    <TableCell colSpan={columns.length + 1} align="center" sx={{ py: 4, borderColor: '#EAECF0' }}>
                      <Typography color="text.secondary">
                        {!hasSearched ? 'Enter criteria and click Search to see results.' : sources.length === 0 ? 'No data sources found. Click "Create Data Source" to add one.' : 'No rows match the current filters.'}
                      </Typography>
                    </TableCell>
                  </TableRow>
                ) : (
                  groupedAndPaginatedData.map((item, index) => {
                        ...
                        <TableRow key={`group-${item.groupKey}-${index}`} sx={{ backgroundColor: '#F5F5F5', '& td': { borderColor: '#EAECF0', fontWeight: 600 } }}>
                          ...
                          >
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              ...
                              <Typography sx={{ fontSize: '0.75rem', color: '#666' }}>({item.count} data source{item.count !== 1 ? 's' : ''})</Typography>
                            </Box>
                          </TableCell>
                        </TableRow>
                      );
                    ...
                    return (
                      <TableRow
                        ...
                        hover
                        selected={selectedRows.includes(row.DATA_SOURCE_ID)}
                        onClick={() => !isEditMode && window.open(`/data-sources/${row.DATA_SOURCE_ID}`, '_blank')}
                        sx={{
                      ...
                      >
                        <TableCell padding="checkbox" onClick={(e) => e.stopPropagation()} sx={{ borderColor: '#EAECF0' }}>
                          <Checkbox
                            checked={selectedRows.includes(row.DATA_SOURCE_ID)}
                            onChange={(e) => {
                              e.stopPropagation();
                              if (e.target.checked) setSelectedRows([...selectedRows, row.DATA_SOURCE_ID]);
                              else setSelectedRows(selectedRows.filter((id) => id !== row.DATA_SOURCE_ID));
                            }}
                            sx={{ padding: '4px' }}
                          />
                        </TableCell>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', p: 2, borderTop: '1px solid #EAECF0', flexWrap: 'wrap', gap: 2 }}>
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
            ...
            >
              {hasDeletedSelected ? 'Restore Data Sources' : 'Delete Data Sources'} ({selectedRows.length})
            </Button>
      ...
      <Dialog open={createDialogOpen} onClose={handleCloseCreateDialog} maxWidth="sm" fullWidth>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <TextField
                fullWidth
                label="Site Address"
                value={createFormData.SITE_ADDRESS}
                onChange={(e) => handleCreateFormChange('SITE_ADDRESS', e.target.value)}
                placeholder="https://..."
                sx={{ '& .MuiOutlinedInput-root': { fontSize: '0.875rem' } }}
              />
            </Grid>
            <Grid item xs={6}>
              <TextField fullWidth type="number" label="LANG_ID" value={createFormData.LANG_ID ?? ''} onChange={(e) => handleCreateFormChange('LANG_ID', e.target.value === '' ? null : e.target.value)} sx={{ '& .MuiOutlinedInput-root': { fontSize: '0.875rem' } }} />
            </Grid>
            <Grid item xs={6}>
              <TextField fullWidth type="number" label="UI_LANG_ID" value={createFormData.UI_LANG_ID ?? ''} onChange={(e) => handleCreateFormChange('UI_LANG_ID', e.target.value === '' ? null : e.target.value)} sx={{ '& .MuiOutlinedInput-root': { fontSize: '0.875rem' } }} />
            </Grid>
            <Grid item xs={6}>
              <FormControl fullWidth size="small" sx={{ '& .MuiOutlinedInput-root': { fontSize: '0.875rem' } }}>
            ...
            <Grid item xs={6}>
              <TextField fullWidth type="number" label="LOG_OPTION_ID" value={createFormData.LOG_OPTION_ID ?? ''} onChange={(e) => handleCreateFormChange('LOG_OPTION_ID', e.target.value === '' ? null : e.target.value)} sx={{ '& .MuiOutlinedInput-root': { fontSize: '0.875rem' } }} />
            </Grid>
            <Grid item xs={4}>
              <FormControlLabel
                control={<Checkbox size="small" checked={createFormData.ALWAYS_ADD_VALUE_IN_LANG === true} onChange={(e) => handleCreateFormChange('ALWAYS_ADD_VALUE_IN_LANG', e.target.checked)} />}
                label="Add value in lang"
                sx={{ '& .MuiFormControlLabel-label': { fontSize: '0.875rem' } }}
              />
            </Grid>
            <Grid item xs={4}>
              <FormControlLabel
                control={<Checkbox size="small" checked={createFormData.ADD_TO_LOG === true} onChange={(e) => handleCreateFormChange('ADD_TO_LOG', e.target.checked)} />}
                label="Add to log"
                sx={{ '& .MuiFormControlLabel-label': { fontSize: '0.875rem' } }}
              />
            </Grid>
            <Grid item xs={4}>
              <FormControlLabel
                control={<Checkbox size="small" checked={createFormData.WIDGET_ACTIVE === true} onChange={(e) => handleCreateFormChange('WIDGET_ACTIVE', e.target.checked)} />}
                label="Widget active"
                sx={{ '& .MuiFormControlLabel-label': { fontSize: '0.875rem' } }}
              />
            </Grid>
        ...
        <DialogActions sx={{ p: 2, borderTop: '1px solid #EAECF0' }}>
          ...
          <Button onClick={handleCreateDataSource} variant="contained" color="primary" disabled={loading} sx={{ textTransform: 'none' }}>Create</Button>
        </DialogActions>
      </Dialog>
